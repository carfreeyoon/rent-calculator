...
# [고정 상수] 잔존가치율 (사용자 수정 불가)
FIXED_INST_RESALE_PCT = 50
FIXED_RENT_RESALE_PCT = 58
...
car_sell_value = int(car_price * (FIXED_INST_RESALE_PCT / 100) * corporate_discount)

# [수정된 인수형 로직] 고정 잔존가치 적용
rent_takeover_price = int(car_price * (FIXED_RENT_RESALE_PCT / 100))
...
with view_col2:
    ...
    <table class="pure-table">
        ...
        <tr class="bg-light font-bold" style="background-color:#e9ecef;"><td>💰 총 투입 비용</td><td>{inst_total_cost_ins:,} 원</td><td>{rent_total_cost_ins:,} 원
